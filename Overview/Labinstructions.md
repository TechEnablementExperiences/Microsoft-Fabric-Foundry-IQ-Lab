...
To address these challenges, Zava’s leadership adopts **Microsoft Fabric + Fabric IQ + Foundry IQ** to unify **data**, **intelligence**, and **AI execution** across the organization.
...
- **Ontology** defines **core business entities** (**Stores, Products, Inventory, Campaigns**) and how they relate  
- **Graph representation** reveals how outcomes such as **stockouts, churn, and revenue** are connected  
...
By the end of the lab, you see how:
...
This enables both **humans and AI systems to reason about data using business language**, not technical data structures.
...
**Task 4.1 –** Create Eventhouse for Real-Time data  
**Task 4.2 –** Ingest Streaming Telemetry into Eventhouse  
**Task 4.3 –** Create Operations Agent (Fabric IQ) and Connect Agent to Ontology  
**Task 4.4 –** Observe Agent Behavior in Real Time

### Exercise 5: Create ontology on top of a semantic model
...
**Task 5.3:** Additional attributes (stream data) to enrich the ontology capabilities
...
**Task 6.3:** Create a Microsoft Foundry agent & add the data agent as an agent
...
|------|---------------|
| Supervisor Agent | Coordinates decisions and routes tasks to the appropriate specialized agent |
| Inventory Agent | Uses Fabric Data Agents to understand stock levels and inventory conditions |
| Store Agent | Evaluates store-level demand and operational constraints |
...
| Return Policy and product guide Documents | Stored in Azure Blob Storage |
...
- The **Sales Associate Agent** answers product questions using **product guides and policy documents**.
...
Together, **Microsoft Fabric, Fabric IQ, and Foundry IQ** create a single platform that connects **data, intelligence, and AI action** across the enterprise. :contentReference[oaicite:0]{index=0}
 ...
 **Task 5.2:** Activate built-in guardrails by enforcing behavioral policies, prompt injection protection, and output moderation, requiring pre-deployment evaluation gates for new agents.  
 **Task 5.3:** Implement an evaluation methodology by defining success metrics, running offline batch evaluations, applying rubric and safety checks, and using telemetry for ongoing online evaluation and progressive rollout