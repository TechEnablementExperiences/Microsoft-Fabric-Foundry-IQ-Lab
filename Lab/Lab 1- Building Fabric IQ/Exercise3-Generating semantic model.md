...
2. Choose the data source within the OneLake catalog, then select the **Lakehouse**: **<inject key= "Lakehouse" enableCopy="false"/>** Lakehouse and click on **Connect**.
...
4. In the table selection window, expand each schema and click on **Select all** to include all available tables, and make sure to deselect **storeinventory** table, and then click on **Confirm**.
   ...
   > **Note**: Ensure that all tables are selected except **storeinventory**, as it is not required for this semantic model. If the tables are not visible, refresh the page and try again.
...
4. Repeat the process to create all required relationships as shown in the table below and click on **Close**.
   ...
   | From Table (Column)              | To Table (Column)        |
    |---------------------------------|--------------------------|
    | campaigns (ProductID)           | product (ProductId)      |
    | clickstream (CustomerID)        | customer (CustomerID)    |
    | clickstream (ProductID)         | product (ProductId)      |
    | feedback (CustomerID)           | customer (CustomerID)    |
    | offlinesales (CustomerID)       | customer (CustomerID)    |
    | offlinesales (ProductID)        | product (ProductId)      |
    | offlinesales (ProductID)        | inventory (ProductID)    |
    | offlinesales (StoreID)          | store (StoreId)          |
    | onlinesales (CustomerID)        | customer (CustomerID)    |
    | onlinesales (ProductID)         | product (ProductId)      |
    | onlinesales (ProductID)         | inventory (ProductID)    |
    | onlinesales (StoreID)           | store (StoreId)          |
    | shiftschedule (StoreID)         | store (StoreId)          |
    | storemanager (StoreID)          | store (StoreId)          |
    | thermostat (StoreId)            | store (StoreId)          |
    | webtraffic (CustomerID)         | customer (CustomerID)    |
    -------------------------------------------------------------
